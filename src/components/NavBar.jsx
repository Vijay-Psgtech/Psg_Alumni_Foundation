...
export default function NavBar() {
  ...
  return (
    ...
    >
            ...
            item.submenu ? (
              <div key={item.label} className="relative" ref={dropdownRef}>
                <button
                  ...
                  text-gray-200 hover:text-blue-300 transition-all duration-300"
                >
                  {item.label}
                  <ChevronDown
                    size={16}
      ...
      {/* MOBILE MENU PANEL */}
      {isOpen && (
        <div className="md:hidden backdrop-blur-xl bg-[#0E0F14]/90 px-6 pb-6 pt-2 flex flex-col gap-4 border-t border-blue-500/20">
          {navItems.map((item) =>
            item.submenu ? (
              <div key={item.label}>
                <button
